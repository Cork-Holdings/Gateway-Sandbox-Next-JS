...
type ApiConfigFormValues = z.infer<typeof apiHeaderSchema>;

const APIHeadersForm = ({ apiID }: { apiID: string }) => {
    const [isLoading, setIsLoading] = useState<boolean>(false);

    const { data: session } = useSession()
    const form = useForm<ApiConfigFormValues>({
        resolver: zodResolver(apiHeaderSchema),
        defaultValues: {
            name: "",
            expectedValue: "",
            isRequired: false,
    ...
    const onSubmit = async (values: ApiConfigFormValues) => {
        ...
        try {
            ...
            const res = await fetch(api_endpoints.backoffice.createApiHeader, {
                method: "POST",
                headers: {
            ...
            <Form {...form}>
                ...
                >
                    <div className="text-center space-y-2">
                        <h1 className="text-2xl text-gray-900 dark:text-white font-bold">API Headers Configuration</h1>
                        <p className="text-gray-600 text-sm dark:text-gray-400">Configure API headers</p>
                    </div>
                    <div className="space-y-6">
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-gray-700 text-sm dark:text-gray-300 font-medium">Header Name</FormLabel>
                                    <FormControl>
                                        <Input
                                            placeholder="For example Content-Type"
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-gray-700 text-sm dark:text-gray-300 font-medium">Expected Value</FormLabel>
                                    <FormControl>
                                        <Input
                                            type="text"
                                            placeholder="For example application/json"
                                            className="border-gray-300 rounded-md w-full dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-blue-500"
                                            {...field}
                                        />
                                    </FormControl>
